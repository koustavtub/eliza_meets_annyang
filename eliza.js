// Chat Bot credit: by George Dunlop, www.peccavi.com
...
function hello() {
    chatter[chatpoint] = "> Hello, I am Listening.. Talk To Me Via Annyang...";
    chatpoint = 1;
    return write();
}
...
function respond() {
    ...
    chatter[chatpoint] += "<br> > " + elizaresponse;
    chatpoint++;
    if (chatpoint >= chatmax) {
        chatpoint = 0;
    }
...
function write() {
    for (i = 0; i < chatmax; i++) {
        n = chatpoint + i;
        if (n < chatmax) {
            document.getElementById('typedtext').innerHTML = '<span class="terminal__text__span">' + chatter[n] + '</span><br />';
        } else {
            document.getElementById('typedtext').innerHTML = '<span class="terminal__text__span">' + chatter[n - chatmax] + '</span><br />';
        }
    }
    refresh();
